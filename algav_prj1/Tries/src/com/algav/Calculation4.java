...
import com.algav.patricia.TestComparaisonConstruction;
...
public class Calculation4 {
	...
	public static double constructionPatriciaFusion(String patname) throws Exception{
		ArrayList<PatriciaTrie> l = TestComparaisonConstruction.allFilleIndirectory(patname);
		 ...
		 for(int cpt = 0; cpt < NTESTSC; cpt++){
			...
			// PatriciaTrie p = new PatriciaTrie(word.get(0));
			TestComparaisonConstruction.allFileFusion(l);
			 long après = System.currentTimeMillis();
			 long écoulé = après - avant;
	         if (écoulé >= 0) {
	            temps_total += écoulé;
